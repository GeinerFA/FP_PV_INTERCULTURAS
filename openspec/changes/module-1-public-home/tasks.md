...
|------|------|-----------|-------|
| 1 | Finalize bilingual content contract and route config | PR 1 | Base slice for messages, nav, routing, service helper |
...
- [x] 1.1 Extend `messages/es.json` and `messages/en.json` with `Home`, `Faqs`, `Navigation`, `Shell`, and updated `Pages.about` keys; keep identical locale structure.
...
- [x] 2.3 Refresh the about-page institutional copy through `messages/es.json` and `messages/en.json` so `src/app/[locale]/(public)/about/page.tsx` carries company information without expanding scope beyond the shared template.
...
- [ ] 4.2 Manually verify `/es` and `/en` for full home section order, featured-program fallback, locale-switch parity, FAQ navigation, `/{locale}#contact`, and `/{locale}/contact` redirect behavior.
- [ ] 4.3 Record the required manual English content review outcome in the change notes before `sdd-apply` is considered production-ready.
